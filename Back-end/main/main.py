...
def main(argv):
    ...
    if argv[1]=="query":
        ...
        if name=="ads_2":
            ...
            list_keyframe = getKeyFrames(ImagePosition,n,919)
        else:
            list_keyframe = getKeyFrames(ImagePosition,n,0)
        n1 = len(list_keyframe)
        if n1<=6 and len(labels)>1:
            label = set(label) & LessKeyFramesClass
            label = list(label)[0]
    else:
        label = "cartoon"
        frames = glob(argv[0]+"/*.jpg")
    ...
    for filename in list_keyframe:
        ...
        # foreground = mask*ImagePosition[filename]
        # mask = mask*255
        mask = np.ones(ImagePosition[filename].shape,np.uint8)*255
        feature = getImageFeatures(mask,ImagePosition[filename])
        # feature = getImageFeatures(foreground)
        foreground = ImagePosition[filename]
